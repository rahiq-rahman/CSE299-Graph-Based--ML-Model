...
def load_karate_regression(train_path, test_path):
    ...
    x = edge_index = targets = train_mask = test_mask = None
    ...
    for fname, data in train_data.items():
        ...
        elif targets is None and is_target_vector(t):
            targets = t.float(); print(f"[Match] targets from: {fname}")
    ...
    if None in [x, edge_index, targets, train_mask, test_mask]:
