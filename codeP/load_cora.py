...
def load_cora(training_dir, testing_dir):
    ...
    x = edge_index = labels = train_mask = test_mask = None
    ...
    for fname, data in train_data.items():
        ...
        elif labels is None and is_label_vector(t):
            labels = t.long()
            print(f"[Match] labels from: {fname}")
    ...
    if None in [x, edge_index, labels, train_mask, test_mask]:
