...
def train_graph_classification(train_loader, test_loader, epochs=20):
    ...
    model = GraphSAGEGraphClassifier(data.num_node_features, 64, int(data.y.max().item()) + 1)
...
class GraphSAGEGraphRegressor(torch.nn.Module):
...
def train_graph_regression(train_loader, test_loader, epochs=20):
    data = next(iter(train_loader))
    model = GraphSAGEGraphRegressor(data.num_node_features, 64)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
...
class GraphSAGEGraphEmbedding(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super().__init__()
        self.conv1 = SAGEConv(input_dim, hidden_dim)
        self.conv2 = SAGEConv(hidden_dim, hidden_dim)
...
def train_graph_embedding(loader, epochs=20):
    ...
    model = GraphSAGEGraphEmbedding(data.num_node_features, 64)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
...
class GraphSAGEGraphSimilarity(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super().__init__()
        self.embedding_net = GraphSAGEGraphEmbedding(input_dim, hidden_dim)
...
def train_graph_similarity(paired_loader, epochs=20):
    ...
    model = GraphSAGEGraphSimilarity(data.num_node_features, 64)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    criterion = torch.nn.MSELoss()
