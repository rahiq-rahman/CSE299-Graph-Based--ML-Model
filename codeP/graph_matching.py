...
def graph_matching_gmn(train_pairs, test_pairs, in_channels=None, epochs=20, lr=0.001):
    in_channels = in_channels or train_pairs[0][0].x.size(1)

    model = GMN(in_channels)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    loss_fn = nn.BCEWithLogitsLoss()
    ...
    for epoch in range(epochs):
        ...
        for g1, g2, label in train_pairs:
            ...
            optimizer.step()
            total_loss += loss.item()
        print(f"[GMN] Epoch {epoch + 1}/{epochs} - Loss: {total_loss:.4f}")
